				     ...
				     __le16 fc, bool mc)
{
	...
	if (ieee80211_is_data(fc)) {
		...
		if (mc){
			tx_cmd->tx_flags &= ~TX_CMD_FLG_STA_RATE_MSK;
			//printk("iwlwifi rate %x rate_n_flags %x \n", info->control.rates[0].idx, tx_cmd->rate_n_flags);	
		}
		else
			return;
	} else if (ieee80211_is_back_req(fc))
		  ...
		  struct sk_buff *skb)
{
	...
	mc = is_multicast_ether_addr(hdr->addr1);
	iwlagn_tx_cmd_build_rate(priv, tx_cmd, info, sta, fc, mc);
